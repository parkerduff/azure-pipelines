...
| 4 | `script: cp target/*.jar ...` (stage artifacts) | `run: cp target/*.jar ...` | Matches ADO root-relative `cp target/*.jar $(Build.ArtifactStagingDirectory)/`. Errors suppressed with `2>/dev/null \|\| true`. |
